...
public class ConfirmOrderService {
    ...
    private void getSeat(Date date,String trainCode,String seatType,String column,List<Integer> offsetList, Integer startIndex, Integer endIndex){
        ...
        //一个车厢一个车厢的获取数据
        for (int i = 0, carriageListSize = carriageList.size(); i < carriageListSize; i++) {
            DailyTrainCarriage dailyTrainCarriage = carriageList.get(i);
            ...
            for (DailyTrainSeat dailyTrainSeat : seatList) {
                ...
                if (StrUtil.isBlank(column)) {
                    ...
                    if (!column.equals(col)) {
                        LOG.info("座位{}列值不对，据徐判断下一个座位，当前列值：{}，目标列值：{}", seatIndex, col, column);
                        continue;
                    }
                }
